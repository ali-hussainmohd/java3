...
//If character not found then returning false
    ...
    //Write a method called isProper
    // in the class KWLinkedList
    // (a class for doubly linked list as discussed in the lectures)
    // that accepts one parameter called elem of type int.
    // If elem exists in the list and the node before
    // elem has data less than elem and the node after elem
    // has data greater than elem, return true.
    // In all other cases return false.
    // If the list is empty or does not contain elem return false.
    // If the elem is in the first node or last node, then also return false.
        public boolean isproper(E elem){
        if(isEmpty()|| head == elem || tail== elem)
            return false;
        Node temp = head;
        while(temp != null){
            if(temp.data.equals(elem))
            {
                int prevNum=((Comparable)temp.prev.data).compareTo(elem),
                    nextNum=((Comparable)temp.next.data).compareTo(elem);
